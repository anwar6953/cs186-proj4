...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
            ...
            int card1, int card2, boolean t1pkey, boolean t2pkey, Map<String, TableStats> stats, Map<String, Integer> tableAliasToId) {
        int card = 1;
        // some code goes here
//        if (joinOp == Predicate.Op.GREATER_THAN){
//        }
//        if (joinOp == Predicate.Op.GREATER_THAN_OR_EQ){
//        }
//        if (joinOp == Predicate.Op.LESS_THAN){
//        }
//        if (joinOp == Predicate.Op.LESS_THAN_OR_EQ){
//        }
...
//        return card <= 0 ? 1 : card;
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        //Replace the following
    	 LogicalJoinNode[] arr = joins.toArray(new LogicalJoinNode[joins.size()]);
         Set<LogicalJoinNode> j = new HashSet<LogicalJoinNode>(Arrays.asList(arr));
         ...
         for (int i = 1; i <= j.size(); i++) {
             ...
             for (Set<LogicalJoinNode> s : subset) {
                 ...
                 for (LogicalJoinNode sp : s) {
                     CostCard cc = computeCostAndCardOfSubplan(stats, filterSelectivities, sp, s, bestPlan.cost, pc);
                     // System.out.println("costcard: " + cc.toString());
         ...
         // System.out.println("pc: " + pc.bestOrders.toString());
         // System.out.println("pc: " + pc.bestOrders.size());
         // System.out.println("j: " + j.toString());
         // System.out.println("return: " + pc.getOrder(j));
         //System.out.println("index: " + new HashSet(Arrays.asList(joins.toArray())).toString());
         //System.out.println("return: " + pc.getOrder(new HashSet(Arrays.asList(joins.toArray()))).toString());
         
         if (explain) {
             printJoins(joins, pc, stats, filterSelectivities);
         }
         Vector<LogicalJoinNode> ret = pc.getOrder(j);
         if (ret == null)
        	 ret = new Vector<LogicalJoinNode>();
         return ret;
